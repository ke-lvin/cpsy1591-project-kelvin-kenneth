...
def run_panel_render():
    ...
    # Precompute scattered dot centers once (ordered farthest-point set).
    max_dots = density_to_count(1.0)
    centers_max = sample_farthest_centers(points, n_centers=max_dots, seed=11)

    # Precompute distances from every vertex to every possible center once.
    # Then density updates only need a fast min over a prefix of centers.
    n_pts = points.shape[0]
    dist2_table = np.empty((n_pts, max_dots), dtype=np.float32)
    center_chunk = 64
    for c0 in range(0, max_dots, center_chunk):
        c1 = min(c0 + center_chunk, max_dots)
        d = points[:, None, :] - centers_max[None, c0:c1, :]
        dist2_table[:, c0:c1] = np.sum(d * d, axis=2)

    def compute_texture(density_value):
        k = density_to_count(density_value)
        min_d2 = np.min(dist2_table[:, :k], axis=1)
        d = np.sqrt(min_d2)

        # Slight radius compensation so dense settings don't overfill.
        radius = np.clip(0.075 * np.sqrt(220.0 / float(k)), 0.052, 0.082)
        edge_softness = 0.010
    ...
    tex_state = {"density": 0.62, "cache": {}}

    def _texture_cache_key(density_value):
        d = int(round(float(np.clip(density_value, 0.0, 1.0)) * 120))
        return d

    def update_texture():
        key = _texture_cache_key(tex_state["density"])
        if key not in tex_state["cache"]:
            tex_state["cache"][key] = compute_texture(tex_state["density"])
        tex_new = tex_state["cache"][key]
